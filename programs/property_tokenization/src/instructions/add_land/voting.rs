...
use crate::{constant::{BUYPROPOSAL, ProposalStatus, VOTERRECIEPT}, errors::ErrorCode, functions::verify_proof, state::{PropertySystemAccount, TransferLandDetail, VoterReciept}};

#[derive(Accounts)]
pub struct VotingToBuy<'info>{
    ...
    #[account(
        constraint = proposal.source_property_system == buyer.key() @ ErrorCode::InvalidProposal
    )]
