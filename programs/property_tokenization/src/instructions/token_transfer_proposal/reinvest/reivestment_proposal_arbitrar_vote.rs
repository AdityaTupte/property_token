...
use crate::{common::{ARBITRAR_RECEIPT_SEEDS, ARBITRAR_REGISTRYSEEDS, ARBITRAR_REINVEST_PROPOSAL_VOTE_RECEIPT_SEEDS, PROPERTY_SYSTEM_SEEDS, USEREINVESTMENTOKEN}, errors::ErrorCode, functions::arbitrar_approval, state::{ArbitratorRecepit, ArbitratorRegistry, ArbitratorVoteReceipts, PropertySystemAccount, TokenTransferProposal, }};

#[derive(Accounts)]
#[instruction(proposal_id : u64,property_system_id:u64)]
pub struct ReinvestArbitrarVote<'info>{
    ...
    #[account(
        ...
        payer = signer,
        seeds=[
            ARBITRAR_REINVEST_PROPOSAL_VOTE_RECEIPT_SEEDS,
            property_system.key().as_ref(), 
            signer.key().as_ref(), 
            proposal.key().as_ref()
        ],
